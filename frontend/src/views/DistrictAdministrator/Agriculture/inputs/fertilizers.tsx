...
const FertilizersPage: React.FC = () => {
  ...
  // Add feedback state/hooks
  const [feedbackOpen, setFeedbackOpen] = useState(false);
  const [feedbackFertilizer, setFeedbackFertilizer] = useState<any>(null);
  const [feedbackText, setFeedbackText] = useState("");
  const [feedbackSubmitting, setFeedbackSubmitting] = useState(false);
  const [fertilizerFeedbacks, setFertilizerFeedbacks] = useState<Record<string, any[]>>({});
  const { userId } = useAuthStore();

  // Fetch feedbacks for all fertilizers on load
  useEffect(() => {
    const fetchAllFeedbacks = async () => {
      const feedbackMap: Record<string, any[]> = {};
      await Promise.all(
        fertilizerTypes.map(async (fertilizer) => {
          const id = fertilizer.id;
          if (!id) return;
          const feedbacks = await fetchFeedbackForCrop(id); // reuse crop feedback API for fertilizers
          feedbackMap[id] = feedbacks;
        })
      );
      setFertilizerFeedbacks(feedbackMap);
    };
    if (fertilizerTypes.length) {
      fetchAllFeedbacks();
    }
  }, [fertilizerTypes]);
  ...
  const handleFeedback = (fertilizer: any) => {
    setFeedbackFertilizer(fertilizer);
    setFeedbackText("");
  ...
  const handleFeedbackSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!feedbackFertilizer || !userId) return;
  ...
  const fertilizerColumns = [
    ...
    { accessorKey: "shortage", header: "Shortage (kg)" },
    {
      ...
      cell: ({ row }: any) => {
        ...
        const fertilizerId = fertilizer.id;
        const feedbacks = fertilizerFeedbacks[fertilizerId] || [];
          ...
          <div className="flex justify-center">
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end">
                ...
                <DropdownMenuItem onClick={() => { setViewFertilizer(fertilizer); setViewOpen(true); }}>
                  <Eye className="h-4 w-4 mr-2 text-blue-600" /> View Details
                </DropdownMenuItem>
                {!userFeedback && (
                  <DropdownMenuItem onClick={() => handleFeedback(fertilizer)}>
            ...
            <Card className="w-full dark:bg-slate-500">
              <TabsContent value="types">
                <Dialog open={viewOpen} onOpenChange={setViewOpen}>
                  <DialogContent>
                    <DialogTitle>Fertilizer Type Details</DialogTitle>
                    {viewFertilizer && (
                ...
                <DataTable columns={fertilizerColumns} data={fertilizerTypes} userType="districtAdministrator" initialLoading={false} />
              </TabsContent>
              <TabsContent value="sources">
                ...
                <DataTable columns={sourceColumns} data={sources} userType="districtAdministrator" initialLoading={false} />
              </TabsContent>
            </Card>
          </Tabs>
        </div>
          ...
          {feedbackFertilizer && (
            <form className="space-y-4" onSubmit={handleFeedbackSubmit}>
              <div className="text-black">
                <b>Fertilizer Type:</b> {feedbackFertilizer.name}
              </div>
              <textarea

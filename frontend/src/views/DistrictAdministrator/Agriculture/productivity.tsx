          ...
          <Tabs value={tab} onValueChange={setTab} className="w-full max-w-6xl space-y-4">
            <Card className="h-[70px] w-full flex items-center pl-[30px] justify-between dark:bg-slate-500">
              <div className="w-full flex justify-between items-center">
                ...
                <button className="bg-[#137775] text-white px-4 py-2 rounded hover:bg-[#0f5c5a] transition-colors">
                  Export Data
                </button>
              </div>
            </Card>
            ...
            <Card className="w-full dark:bg-slate-500">
              <TabsContent value="overview">
                <div className="bg-white rounded-lg shadow-sm border p-6 mb-8">
                  <h2 className="text-lg font-semibold text-[#137775] mb-4">District Productivity by Crop</h2>
              ...
              <TabsContent value="by-crop">
                <div className="bg-white rounded-lg shadow-sm border p-6 mb-8">
                  <h2 className="text-lg font-semibold text-[#137775] mb-4">Productivity Analysis by Crop</h2>
                  <div className="overflow-x-auto">
                    <table className="min-w-full divide-y divide-gray-200">
                      ...
                      <tbody className="bg-white divide-y divide-gray-200">
                        {byCrop.map((row, index) => (
                          <tr key={row.crop} className={index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                            ...
                            <td className="px-6 py-4 whitespace-nowrap text-sm">
                              <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                                row.yield >= 3.0 ? 'bg-green-100 text-green-800' : 
                                row.yield >= 2.0 ? 'bg-yellow-100 text-yellow-800' : 
                                'bg-red-100 text-red-800'
                              }`}>
                                {row.yield >= 3.0 ? 'High' : row.yield >= 2.0 ? 'Medium' : 'Low'}
                              </span>
                            </td>
                          </tr>
                        ))}
              ...
              <TabsContent value="by-region">
                <div className="bg-white rounded-lg shadow-sm border p-6 mb-8">
                  <h2 className="text-lg font-semibold text-[#137775] mb-4">Productivity Analysis by Region</h2>
                  <div className="overflow-x-auto">
                    <table className="min-w-full divide-y divide-gray-200">
                      <thead className="bg-gray-50">
                        <tr>
                      ...
                      <tbody className="bg-white divide-y divide-gray-200">
                        {byRegion.map((row, index) => (
                          <tr key={row.region} className={index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                            ...
                            <td className="px-6 py-4 whitespace-nowrap text-sm">
                              <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                                row.yield >= 3.3 ? 'bg-green-100 text-green-800' : 
                                row.yield >= 3.0 ? 'bg-yellow-100 text-yellow-800' : 
                                'bg-red-100 text-red-800'
                              }`}>
                                {row.yield >= 3.3 ? 'Excellent' : row.yield >= 3.0 ? 'Good' : 'Needs Improvement'}
                              </span>
                            </td>
                          </tr>
                        ))}
                ...
                <div className="flex justify-between items-center">
                  <div className="text-sm text-gray-600">
                    Showing {byRegion.length} regions
                  </div>
                </div>
              </TabsContent>
            </Card>

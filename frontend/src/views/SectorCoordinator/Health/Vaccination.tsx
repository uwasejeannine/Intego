...
const Vaccination: React.FC = () => {
  ...
  const [newCampaign, setNewCampaign] = useState({ name: "", date: "", coverage: "", target: "" });
  ...
  useEffect(() => {
    ...
    // TODO: Fetch campaigns from backend if endpoint exists
  }, []);

  const handleAddVacc = async (e: React.FormEvent) => {
    e.preventDefault();
    const res = await fetch(VACCINES_API, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        ...newVacc,
        stock: Number(newVacc.stock),
        coverage: newVacc.coverage ? Number(newVacc.coverage) : undefined,
        target: newVacc.target ? Number(newVacc.target) : undefined
      })
    });
    if (res.ok) {
      const created = await res.json();
      setVaccinations([...vaccinations, created]);
      setAddOpen(false);
      setNewVacc({
        name: "",
        type: "",
  ...
  const handleAddCampaign = async (e: React.FormEvent) => {
    ...
    const newCampaignData = {
      ...newCampaign,
      coverage: Number(newCampaign.coverage),
      target: Number(newCampaign.target)
    };
    setCampaigns([...campaigns, { ...newCampaignData, id: Date.now() }]);
    setAddCampaignOpen(false);
    setNewCampaign({ name: "", date: "", coverage: "", target: "" });
  };
  ...
  const handleAddRecord = async (e: React.FormEvent) => {
    e.preventDefault();
    const res = await fetch(VACC_RECORDS_API, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        ...newRecord,
        dose_number: Number(newRecord.dose_number)
      })
    });
    if (res.ok) {
      const created = await res.json();
      setVaccinationRecords([...vaccinationRecords, created]);
      setAddRecordOpen(false);
      setNewRecord({ patient_name: "", patient_id: "", vaccine_id: "", facility_id: "", date_administered: "", dose_number: "", notes: "" });
    }
  };
  ...
  // DataTable columns for campaigns
    ...
    { accessorKey: "target", header: "Target (%)" },
    {
      ...
      cell: ({ row }: any) => (
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end">
            <DropdownMenuLabel>Actions</DropdownMenuLabel>
            <DropdownMenuItem onClick={() => { setViewCampaign(row.original); setViewCampaignOpen(true); }}>
              <Eye className="h-4 w-4 mr-2 text-blue-600" /> View Details
            </DropdownMenuItem>
            <DropdownMenuItem onClick={() => { setEditCampaign(row.original); setEditCampaignOpen(true); }}>
              <Pencil className="h-4 w-4 mr-2 text-yellow-600" /> Edit
            </DropdownMenuItem>
            <DropdownMenuItem onClick={() => handleDeleteCampaign(row.original)}>
              <Trash2 className="h-4 w-4 mr-2 text-red-600" /> Delete
            </DropdownMenuItem>
          </DropdownMenuContent>
        </DropdownMenu>
                ...
                <div className="flex justify-items-end">
                  {tab === "records" && (
                    <Dialog open={addRecordOpen} onOpenChange={setAddRecordOpen}>
                      <DialogTrigger asChild>
                        <Button className="bg-[#137775] text-white">Add Vaccination Record</Button>
                      </DialogTrigger>
                      <DialogContent>
                        <DialogTitle>Add Vaccination Record</DialogTitle>
                        <form className="space-y-4" onSubmit={handleAddRecord}>
                          <input className="w-full border rounded p-2 text-black" placeholder="Patient Name" value={newRecord.patient_name} onChange={e => setNewRecord({ ...newRecord, patient_name: e.target.value })} required />
                          <input className="w-full border rounded p-2 text-black" placeholder="Patient ID" value={newRecord.patient_id} onChange={e => setNewRecord({ ...newRecord, patient_id: e.target.value })} required />
                          <select className="w-full border rounded p-2 text-black" value={newRecord.vaccine_id} onChange={e => setNewRecord({ ...newRecord, vaccine_id: e.target.value })} required>
                            <option value="">Select Vaccine</option>
                            {vaccinations.map(v => <option key={v.id} value={v.id}>{v.name}</option>)}
                          </select>
                          <select className="w-full border rounded p-2 text-black" value={newRecord.facility_id} onChange={e => setNewRecord({ ...newRecord, facility_id: e.target.value })} required>
                            <option value="">Select Facility</option>
                            {facilities.map(f => <option key={f.id} value={f.id}>{f.name}</option>)}
                          </select>
                          <input className="w-full border rounded p-2 text-black" placeholder="Date Administered" type="date" value={newRecord.date_administered} onChange={e => setNewRecord({ ...newRecord, date_administered: e.target.value })} required />
                          <input className="w-full border rounded p-2 text-black" placeholder="Dose Number" type="number" value={newRecord.dose_number} onChange={e => setNewRecord({ ...newRecord, dose_number: e.target.value })} required />
                          <textarea className="w-full border rounded p-2 text-black" placeholder="Notes" value={newRecord.notes} onChange={e => setNewRecord({ ...newRecord, notes: e.target.value })} />
                  ...
                  {tab === "vaccines" && (
                    <Dialog open={addOpen} onOpenChange={setAddOpen}>
                      <DialogTrigger asChild>
                        <Button className="bg-[#137775] text-white">Add Vaccination</Button>
                      </DialogTrigger>
                      <DialogContent>
                        <DialogTitle>Add New Vaccination</DialogTitle>
                        <div className="max-h-[70vh] overflow-y-auto">
                          <form className="space-y-4" onSubmit={handleAddVacc}>
                            <input className="w-full border rounded p-2 text-black" placeholder="Vaccine Name" value={newVacc.name} onChange={e => setNewVacc({ ...newVacc, name: e.target.value })} required />
                            <input className="w-full border rounded p-2 text-black" placeholder="Type" value={newVacc.type} onChange={e => setNewVacc({ ...newVacc, type: e.target.value })} required />
                            <input className="w-full border rounded p-2 text-black" placeholder="Manufacturer" value={newVacc.manufacturer} onChange={e => setNewVacc({ ...newVacc, manufacturer: e.target.value })} required />
                            <input className="w-full border rounded p-2 text-black" placeholder="Stock" type="number" value={newVacc.stock} onChange={e => setNewVacc({ ...newVacc, stock: e.target.value })} required />
                            <input className="w-full border rounded p-2 text-black" placeholder="Expiry Date" type="date" value={newVacc.expiry_date} onChange={e => setNewVacc({ ...newVacc, expiry_date: e.target.value })} required />
                            <input className="w-full border rounded p-2 text-black" placeholder="Batch Number" value={newVacc.batch_number} onChange={e => setNewVacc({ ...newVacc, batch_number: e.target.value })} required />
                            <select className="w-full border rounded p-2 text-black" value={newVacc.status} onChange={e => setNewVacc({ ...newVacc, status: e.target.value })} required>
                              <option value="active">Active</option>
                              <option value="inactive">Inactive</option>
                            </select>
                            <input className="w-full border rounded p-2 text-black" placeholder="Coverage (%)" type="number" value={newVacc.coverage} onChange={e => setNewVacc({ ...newVacc, coverage: e.target.value })} />
                            <input className="w-full border rounded p-2 text-black" placeholder="Target (%)" type="number" value={newVacc.target} onChange={e => setNewVacc({ ...newVacc, target: e.target.value })} />
                            <input className="w-full border rounded p-2 text-black" placeholder="Last Campaign Date" type="date" value={newVacc.last_campaign} onChange={e => setNewVacc({ ...newVacc, last_campaign: e.target.value })} />
                            <Button type="submit" className="bg-[#137775] text-white">Add</Button>
                          </form>
                        </div>
                      </DialogContent>
                  ...
                  {tab === "campaigns" && (
                    <Dialog open={addCampaignOpen} onOpenChange={setAddCampaignOpen}>
                      <DialogTrigger asChild>
                        <Button className="bg-[#137775] text-white">Add Campaign</Button>
                      </DialogTrigger>
                      <DialogContent>
                        <DialogTitle>Add New Campaign</DialogTitle>
                        <form className="space-y-4" onSubmit={handleAddCampaign}>
                          <input className="w-full border rounded p-2 text-black" placeholder="Campaign Name" value={newCampaign.name} onChange={e => setNewCampaign({ ...newCampaign, name: e.target.value })} required />
                          <input className="w-full border rounded p-2 text-black" placeholder="Date" type="date" value={newCampaign.date} onChange={e => setNewCampaign({ ...newCampaign, date: e.target.value })} required />
                          <input className="w-full border rounded p-2 text-black" placeholder="Coverage (%)" type="number" value={newCampaign.coverage} onChange={e => setNewCampaign({ ...newCampaign, coverage: e.target.value })} required />
                          <input className="w-full border rounded p-2 text-black" placeholder="Target (%)" type="number" value={newCampaign.target} onChange={e => setNewCampaign({ ...newCampaign, target: e.target.value })} required />
                          <Button type="submit" className="bg-[#137775] text-white">Add</Button>
                        </form>
                      </DialogContent>
                    </Dialog>
              ...
              <TabsContent value="campaigns">
                <Dialog open={viewCampaignOpen} onOpenChange={setViewCampaignOpen}>
                  <DialogContent>
                    ...
                    {viewCampaign && (
                      <div className="space-y-2 text-black">
                        <div><b>Campaign:</b> {viewCampaign.name}</div>
                        <div><b>Date:</b> {viewCampaign.date}</div>
                        <div><b>Coverage:</b> {viewCampaign.coverage}%</div>
                        <div><b>Target:</b> {viewCampaign.target}%</div>
                      </div>
                    )}
                  </DialogContent>
                </Dialog>
                <Dialog open={editCampaignOpen} onOpenChange={setEditCampaignOpen}>
                  <DialogContent>
                    ...
                    {editCampaign && (
                      <form className="space-y-4" onSubmit={handleEditCampaign}>
                        <input className="w-full border rounded p-2 text-black" placeholder="Campaign Name" value={editCampaign.name} onChange={e => setEditCampaign({ ...editCampaign, name: e.target.value })} required />
                        <input className="w-full border rounded p-2 text-black" placeholder="Date" type="date" value={editCampaign.date} onChange={e => setEditCampaign({ ...editCampaign, date: e.target.value })} required />
                        <input className="w-full border rounded p-2 text-black" placeholder="Coverage (%)" type="number" value={editCampaign.coverage} onChange={e => setEditCampaign({ ...editCampaign, coverage: e.target.value })} required />
                        <input className="w-full border rounded p-2 text-black" placeholder="Target (%)" type="number" value={editCampaign.target} onChange={e => setEditCampaign({ ...editCampaign, target: e.target.value })} required />
                        <Button type="submit" className="bg-[#137775] text-white">Save</Button>
                      </form>
                    )}
                  </DialogContent>
                </Dialog>
                <DataTable columns={campaignColumns} data={campaigns} userType="sectorCoordinator" initialLoading={false} />
              </TabsContent>
